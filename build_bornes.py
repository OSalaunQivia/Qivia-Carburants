...
def build_geojson(df: pd.DataFrame) -> dict:
    ...
    # Clé d'unicité station : id_station_itinerance VALIDE (commence par "FR"),
    # sinon repli sur coordonnées + nom/adresse. Cela évite d'agglomérer en une
    # seule fausse station toutes les lignes au même id factice ("Non concerné",
    # "Non renseigné", etc.).
    def _norm(series):
        return (series.astype(str).str.strip().str.lower()
                .replace({"nan": "", "none": "", "non concerné": "", "non renseigné": ""}))

    coord_key = df["lat"].round(5).astype(str) + "," + df["lng"].round(5).astype(str)
    fallback_key = coord_key
    if nom_col is not None:
        fallback_key = fallback_key + "|" + _norm(df[nom_col])
    elif adr_col is not None:
        fallback_key = fallback_key + "|" + _norm(df[adr_col])

    if id_col:
        ids = df[id_col].astype(str).str.strip()
        valid = ids.str.upper().str.startswith("FR") & (ids.str.len() >= 5)
        df["_key"] = fallback_key
        df.loc[valid, "_key"] = ids[valid].str.upper()
        print(f"[irve] ids valides : {int(valid.sum()):,} / {len(df):,} lignes")
    else:
        df["_key"] = fallback_key
