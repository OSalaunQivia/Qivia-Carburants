...
def generate_carte(stations_with_prices, template_path="qivia_carte_template.html", output_path="qivia_carte.html"):
    ...
    # keyed by BOTH uuid AND provider_id to match Overpass ref:FR:prix-carburants
    price_lookup = {}
    for s in stations_with_prices:
        prices = {FUELS_FR[f]: s[f] for f in list(FUELS_FR.keys()) if s.get(f) is not None}
        if not prices: continue
        # Key by uuid
        uid = _clean(str(s.get("uuid",""))).split(".")[0]
        if uid: price_lookup[uid] = prices
        # Key by provider_id number (matches Overpass ref:FR:prix-carburants)
        pid = _clean(str(s.get("provider_id",""))).split(".")[0]
        if pid: price_lookup[pid] = prices

    print(f"[Carte] {len(price_lookup)} entrées prix (uuid + provider_id)")
    ...
    if os.path.exists(OVERPASS_BRAND_FILE):
        try:
            ...
            for _, row in df_ov.iterrows():
                ...
                # Try both postal columns
                postal = _clean(str(row.get(postal_col_ov, "") or row.get('tags/postal_code', ""))).split(".")[0].zfill(5)
                # Try both city columns
                city = _clean(str(row.get(city_col, "") or row.get('tags/addr:full', "")))
                # Try both address columns
                address = _clean(str(row.get(addr_col_ov, "") or row.get('tags/address', "")))
    ...
    # Always add stations from CSV that are NOT already in Overpass
    # (some stations report prices but aren't in OSM)
    existing_refs = set(s.get("uuid","") for s in all_stations if s.get("uuid"))
    csv_added = 0
    for s in stations_with_prices:
        if not s.get("lat") or not s.get("lng"): continue
        pid = _clean(str(s.get("provider_id",""))).split(".")[0]
        uid = _clean(str(s.get("uuid",""))).split(".")[0]
        # Skip if already in Overpass
        if pid in existing_refs or uid in existing_refs: continue
        prices = {FUELS_FR[f]: s[f] for f in list(FUELS_FR.keys()) if s.get(f) is not None}
        ...
        if uid: existing_refs.add(uid)
        if pid: existing_refs.add(pid)
        csv_added += 1

    if csv_added > 0:
        print(f"[Carte] +{csv_added} stations CSV non présentes dans Overpass")

    # === RÉSEAU DKV ===
    # Étape 1: utiliser les provider_ids déjà calculés par fetch_dkv_ids()
    dkv_provider_ids = fetch_dkv_ids()
    dkv_matched = 0
    dkv_added = 0

    # Flaguer les stations Overpass dont ref = provider_id DKV
    for s in all_stations:
        uid = str(s.get("uuid","")).strip()
        if uid and uid in dkv_provider_ids:
            if 'dkv' not in s["networks"]:
                s["networks"].append("dkv")
                dkv_matched += 1
    print(f"[Carte] DKV step1 (provider_id): {dkv_matched} stations")

    # Étape 2: stations DKV non trouvées → matching adresse puis géocodage
        ...
        try:
            ...
            if df_dkv is not None:
                ...
                for _, row in df_dkv.iterrows():
                    ...
                    city   = str(row[city_col_dkv]) if city_col_dkv else ""
                    brand  = str(row[brand_col_dkv]) if brand_col_dkv else ""
                    candidates = postal_lookup.get(postal, [])

                    # Skip if already DKV
                    if any('dkv' in all_stations[i]["networks"] for i in candidates):
                        continue
                    ...
                    if best_score >= 0.2 and best_i is not None:
                        ...
                        # Géocoder et ajouter
                        lat_g, lon_g = _geocode(addr, postal, city, geocode_cache)
                        if lat_g and lon_g:
                            new_st = {
                            ...
                            idx_new = len(all_stations)
                            all_stations.append(new_st)
                            if postal not in postal_lookup:
                                postal_lookup[postal] = []
                            postal_lookup[postal].append(idx_new)
                            dkv_added += 1

                _save_geocode_cache(geocode_cache)
                total_dkv = sum(1 for s in all_stations if 'dkv' in s["networks"])
                print(f"[Carte] DKV step2: {dkv_addr_matched} adresse, {dkv_added} géocodées → {total_dkv} total DKV")
        except Exception as e:
            print(f"[Carte] Erreur DKV: {e}")
            import traceback; traceback.print_exc()
    else:
        print(f"[Carte] {DKV_STATIONS_FILE} introuvable")
