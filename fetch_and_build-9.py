...
def generate_geojson(api_df, output_path=GEOJSON_OUT):
    ...
    if os.path.exists(EXCEL_FILE):
        ...
        xl['ID'] = xl['ID'].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)
    ...
    for _, row in api_df.iterrows():
        ...
        props = {
            ...
            'cp':          str(row.get('postal','')).split('.')[0],
            'departement': (lambda d: 'Corse-du-Sud' if str(d or '').strip() in ('20','2a','2A') and float(row.get('latitude') or 0) < 41.86
                            else 'Haute-Corse' if str(d or '').strip() in ('20','2b','2B') and float(row.get('latitude') or 0) >= 41.86
                            else _val(d))(_val(row.get('department'))),
