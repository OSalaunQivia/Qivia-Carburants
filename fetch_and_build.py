...
def generate_carte(stations, template_path="qivia_carte_template.html", output_path="qivia_carte.html"):
    """Regénère qivia_carte.html avec toutes les stations + flags réseau (qivia/dkv/total)."""
    import json as _json, re as _re, glob as _glob
    ...
    # Build station list from JSON data
    all_stations = []
    postal_lookup = {}
    ...
    for s in stations:
        ...
        station = {
            ...
            "networks":   ["qivia"],  # toutes les stations sont Qivia
        }
    ...
    print(f"[Carte] {len(all_stations)} stations de base (Qivia)")
    ...
    if os.path.exists(DKV_STATIONS_FILE):
        ...
        try:
            ...
            if df_dkv is not None:
                cols = [c.lower().strip() for c in df_dkv.columns]
                addr_col   = df_dkv.columns[[i for i,c in enumerate(cols) if 'adresse' in c or 'address' in c][0]] if any('adresse' in c or 'address' in c for c in cols) else df_dkv.columns[2]
                postal_col = df_dkv.columns[[i for i,c in enumerate(cols) if 'postal' in c or 'cp' in c][0]] if any('postal' in c or 'cp' in c for c in cols) else df_dkv.columns[3]

                for _, row in df_dkv.iterrows():
                    postal = str(row[postal_col]).split('.')[0].zfill(5)
                    dkv_addr = _normalize(str(row[addr_col]))
                    dkv_words = set(dkv_addr.split()) - NOISE
                    ...
                    for i in candidates:
                        score = _addr_score(str(row[addr_col]), all_stations[i]["address"])
                        if score > best_score:
                ...
                print(f"[Carte] DKV: {dkv_matched} stations identifiées")
        except Exception as e:
    ...
    TOTAL_BRANDS_RE = re.compile(r'total|elan|as24|access', re.IGNORECASE)
    total_matched = 0
