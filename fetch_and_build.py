...
TOTAL_BRANDS_OSM = {'total', 'totalenergies', 'total access', 'total express', 'elan', 'as24', 'access'}
...
def fetch_total_ids():
    ...
    try:
        ...
        # Filter TotalEnergies network brands
        brand_col = 'tags/brand' if 'tags/brand' in df.columns else 'brand'
        ref_col = 'tags/ref:FR:prix-carburants' if 'tags/ref:FR:prix-carburants' in df.columns else 'ref'
        ...
        ids.discard('')
        ids.discard('nan')
        print(f"[Total] {mask.sum()} stations OSM → {len(ids)} IDs provider")
        print(f"[Total] Marques: {df[mask][brand_col].value_counts().to_dict()}")
        return ids
    except Exception as e:
...
def fetch_dkv_ids():
    ...
    try:
        r = requests.get(DKV_PDF_URL, timeout=60)
        r.raise_for_status()
        # Extract DKV numbers from PDF binary content (text search)
        text = r.content.decode("latin-1", errors="ignore")
        ids = set(re.findall(r'DKV No[: ]+(\d{6,8})', text))
        if not ids:
            # Try alternate pattern in raw bytes
            ids = set(re.findall(r'(\d{7})', text))
        print(f"[DKV] {len(ids)} IDs extraits du PDF")
        with open(DKV_IDS_CACHE, "w") as f:
            f.write("\n".join(sorted(ids)))
        return ids
    except Exception as e:
        print(f"[DKV] Erreur téléchargement PDF: {e}")
