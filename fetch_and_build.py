    ...
    if os.path.exists(OVERPASS_BRAND_FILE):
        try:
            ...
            for _, row in df_ov.iterrows():
                lat = row.get(lat_col)
                lon = row.get(lon_col)
                # Try center coords for ways
                if (_pd.isna(lat) or lat == 0) and 'center/lat' in df_ov.columns:
                    lat = row.get('center/lat')
                    lon = row.get('center/lon')
                if _pd.isna(lat) or _pd.isna(lon) or lat == 0 or lon == 0:
                    continue
            ...
            if df_dkv is not None:
                ...
                for _, row in df_dkv.iterrows():
                    ...
                    if best_score >= 0.2 and best_i is not None:
                        ...
                        # Station DKV not in Overpass - add it with city/postal only (no coords available)
                        # Skip for now - we cannot place it without coordinates
                        pass

                print(f"[Carte] DKV: {dkv_matched} stations identifiées ({dkv_added} ajoutées)")
        except Exception as e:
