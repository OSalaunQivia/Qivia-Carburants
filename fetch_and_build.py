...
def main():
    ...
    rows      = fetch_api()
    if not rows:
        print("[API] 0 stations — tentative de chargement du dernier CSV disponible...")
        last_csv = latest_csv()
        if last_csv:
            import pandas as _pd
            df = _pd.read_csv(last_csv, dtype=str)
            print(f"[API] Fallback sur {last_csv} ({len(df)} lignes)")
        else:
            print("[API] Aucun CSV disponible — abandon.")
            return
    else:
